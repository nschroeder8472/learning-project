...
- **Adjacency List**: HashMap where each vertex maps to a vector of its neighbors (most common)
- **Adjacency Matrix**: 2D vector where matrix[i][j] indicates edge from i to j
- **Edge List**: Vector of all edges as (source, destination) tuples

### Depth-First Search (DFS)
- **Approach**: Explore as far as possible along each branch before backtracking
- **Implementation**: Can use recursion (implicit stack) or explicit stack (Vec)
...
- **Implementation**: Uses a queue (VecDeque)
...
- **Uses a priority queue** (BinaryHeap) to always process the nearest unvisited vertex
...
1. **DFS**: Use recursion or Vec for stack. Mark vertices as visited using HashSet. Process vertex, then recursively visit unvisited neighbors.
2. **BFS**: Use VecDeque for queue. Start with source vertex, enqueue it. While queue not empty: dequeue, process, enqueue unvisited neighbors.
...
6. **Dijkstra's Algorithm**: Use BinaryHeap for priority queue. Start with source at distance 0. Pop closest vertex, update distances to neighbors, add to heap.

## Testing Your Implementation

Run all graph traversal tests:
```bash
cargo test --package algorithms graph_traversal
```

Run specific test:
```bash
cargo test --package algorithms ex1_depth_first_search
```
