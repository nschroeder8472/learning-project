...
### Easy
1. **two_sum** - Find two numbers in a slice that add up to a target value
2. **contains_duplicate** - Check if a slice contains any duplicate elements
...
**two_sum**: Use a HashMap to store numbers you've seen. For each number, check if (target - number) exists in the map.

**contains_duplicate**: Use a HashSet to track elements you've seen. If you encounter an element already in the set, you've found a duplicate.
...
**longest_substring_without_repeating**: Use sliding window technique with a HashSet. Expand window by adding characters, shrink when you find a duplicate.

**design_hash_map**: Use a vector of linked lists (buckets). Hash function determines which bucket. Handle collisions with chaining.
