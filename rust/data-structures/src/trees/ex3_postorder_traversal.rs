...
pub fn postorder(root: Option<Box<TreeNode>>) -> Vec<i32> {
    ...
    // Hint: Use recursion - traverse left, traverse right, visit node
...
mod tests {
    ...
    #[test]
    fn test_empty_tree() {
        assert_eq!(postorder(None), vec![]);
    }

    #[test]
    fn test_single_node() {
        let root = Some(Box::new(TreeNode::new(1)));
        assert_eq!(postorder(root), vec![1]);
    }
    ...
    fn test_left_skewed_tree() {
        ...
        assert_eq!(postorder(root), vec![1, 2, 3]);
    }
    ...
    fn test_complex_tree() {
        ...
        let node2 = Some(Box::new(TreeNode::with_children(2, node4, node5)));
        let root = Some(Box::new(TreeNode::with_children(1, node2, node3)));
        assert_eq!(postorder(root), vec![4, 5, 2, 6, 7, 3, 1]);
    }
    ...
    fn test_only_left_children() {
        ...
        assert_eq!(postorder(root), vec![2, 1, 3]);
    }
}
