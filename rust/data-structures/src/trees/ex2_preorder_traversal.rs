...
pub fn preorder(root: Option<Box<TreeNode>>) -> Vec<i32> {
    ...
    // Hint: Use recursion - visit node, traverse left, traverse right
...
mod tests {
    ...
    #[test]
    fn test_empty_tree() {
        assert_eq!(preorder(None), vec![]);
    }

    #[test]
    fn test_single_node() {
        let root = Some(Box::new(TreeNode::new(1)));
        assert_eq!(preorder(root), vec![1]);
    }
    ...
    fn test_left_skewed_tree() {
        ...
        assert_eq!(preorder(root), vec![3, 2, 1]);
    }
    ...
    fn test_right_skewed_tree() {
        ...
        assert_eq!(preorder(root), vec![1, 2, 3]);
    }
    ...
    fn test_complex_tree() {
        ...
        let node2 = Some(Box::new(TreeNode::with_children(2, node4, node5)));
        let root = Some(Box::new(TreeNode::with_children(1, node2, node3)));
        assert_eq!(preorder(root), vec![1, 2, 4, 5, 3, 6, 7]);
    }
